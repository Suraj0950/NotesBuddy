...
import SavedNotesContextProvider from "./context/SavedNotesContextProvider"
...
export default function UserPage() {
  ...
  //const { savedNotes, setSavedNotes } = useContext(SavedNotesContextProvider);
  ...
  return (
    <div className="min-h-screen">
      {/* Fixed sidebar */}
      <div className="fixed hidden md:flex flex-col justify-around left-0 top-10 px-3 h-screen w-85 bg-linear-to-b from-sky-400/10 via-sky-300/10 to-purple-500/10 backdrop-blur-lg shadow-2xl  border-white/20 rounded-r-2xl z-10">
        <div className="p-4 mt-5">
          <div className="mb-6">
            <div className="w-22 h-22 mx-auto bg-gray-200 rounded-full flex items-center justify-center mb-2">
              {user?.photoURL ? (
                <img
            ...
            <h2 className="text-xl font-ssemibold text-center">{user?.displayName || "User"}</h2>
            <p className="text-sm text-gray-600 text-center">{user?.email || "loading..."}</p>
          </div>
          <nav className="space-y-2">
            <button
              onClick={() => setActiveTab("saves")}
              className={`w-full text-left p-3 rounded-xl flex items-center ${activeTab === "saves" ? "bg-purple-100 backdrop-blur-lg" : "hover:bg-slate-200"}`}
            >
              <Bookmark size={18} className="mr-2" /> Saved Notes
            </button>
            <button
              onClick={() => setActiveTab("uploads")}
              className={`w-full text-left p-3 rounded-xl flex items-center ${activeTab === "uploads" ? "bg-purple-100 backdrop-blur-lg" : "hover:bg-slate-200"}`}
            >
        ...
        <div
          className="px-25  py-3 text-red-500 border bg-purple-500/10 flex justify-start items-center rounded-xl transition-all font-semibold cursor-pointer"
      ...
      <div className="md:hidden flex mt-20  justify-between items-center p-5 bg-amber-50 shadow-sm">
        <button
          onClick={() => setActiveTab("uploads")}
